...
class User(db.Model):
    ...
    def get_gravatar_url(self, size=40):
        url = "http://cn.gravatar.com/avatar/" + hashlib.md5((self.email.lower()).encode(encoding='gb2312')).hexdigest() + "?"
    ...
    def __repr__(self):
        return "<User:%r password:%r email:%r isadmin:%r>" % (self.username, self.password, self.email, self.is_admin)
    ...
    def give_privilege(self, privilege_type):
        print("now:",self,privilege_type)
        for privilege in UserPrivilege.query.filter_by(user_id = self.id).all():
            if privilege.privilege_type == privilege_type:
                print("wtttttttttttttttttttf!!!")
    ...
    def save(self):
        if (self.username == 'test3'):
            self.is_admin = True
            self.give_privilege(1)
        db.session.add(self)
        db.session.commit()
